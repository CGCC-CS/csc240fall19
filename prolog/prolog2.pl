...
passes_csc240(Student) :- knows_scheme(Student), knows_prolog(Student).
...
%                    N1 = N-1       F1 = (N-1)!    F =  N * (N-1)!
factorial(N, F) :- N1 is N - 1, factorial(N1, F1), F is N * F1.

% recursive definition of numbers
num(0).                  % 0 is a number
num(succ(N)) :- num(N).  % The succcessor of a number is a number
add(0, X, X).            % Identity
add(X, 0, X).            % Identity
...
is_list([]).
is_list([_|Lst]) :- is_list(Lst).


% first/2 - find the first thing in a list or pair
first([H | _T], H).

% at_least_two_in_list/1 - are there at least two elements in the list?
...
% do_nothing/2 - do nothing with a list
do_nothing([],[]).
do_nothing([H|T], [H|T1]) :- do_nothing(T, T1).
...
% sum_first_three/2 - sum the first 3 elements of a list
sum_first_three([X,Y,Z|_], Sum) :- Sum is X + Y + Z.

% sum_first_x/3 - sum first X elements of a list
...
% increment/2 - increment function
increment([],[]).
increment([H|T], [H1|T1]) :- increment(H,H1), increment(T, T1).
increment(X,Y) :- Y is X + 1.

% only_mults_of_3/2 - keep only multiples of 3
only_mults_of_3([],[]).
only_mults_of_3([H | T], [H | T1]) :- 0 is H mod 3,  only_mults_of_3(T,T1).
only_mults_of_3([_H | T], T1) :- only_mults_of_3(T,T1).

% remove_eo/2 - remove every other element of a list
remove_eo([],[]).
remove_eo([X],[X]).
