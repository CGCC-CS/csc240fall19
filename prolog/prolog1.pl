...
% avg_temp_celsius/2 - caluclate the average temperature in Celsius
...
% To check that two variables are equal, USE THE SAME VARIABLE NAME 
...
score(washington, 115).
score(detroit, 99).
score(new_england, 34).
score(atlanta, 28).

won(X) :- score(X, ScoreX), score(_Y, ScoreY), ScoreX > ScoreY.

% better rule
game(washington, 115, detroit, 99).
game(new_england, 34, atlanta, 28).
